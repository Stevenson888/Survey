...
<!--            size="small"-->
<!--            @click="updateUserStatusBtn(scope.$index, scope.row)"-->
<!--            v-if="user.roleFlag === 'ROLE_DEV'"-->
...
<!--            type="danger"-->
<!--            size="small"-->
<!--            @click="userInactivateBtn(scope.row)"-->
<!--            v-if="user.roleFlag === 'ROLE_DEV' "-->
<!--          >-->
<!--            <template #default v-if="scope.row.status === 1"> 禁用 </template>-->
...
<!--            type="danger"-->
<!--            size="small"-->
<!--            @click="userActivateBtn(scope.row)"-->
<!--            v-if="user.roleFlag === 'ROLE_DEV' && scope.row.status == 0"-->
<!--          >激活</el-button>-->
          ...
          <el-button
            ...
            size="small"
            @click="userInactivateBtn(scope.row)"
            v-if="user.roleFlag === 'ROLE_DEV' && scope.row.status == 1"
          >禁用</el-button>
          <el-button
            icon="el-icon-edit"
            type="warning"
...
<!--    <el-dialog title="激活用户" :visible.sync="userActivateDialogFormVisible" width="35%" :close-on-click-modal="false">-->
<!--      <div>-->
<!--        确定激活用户吗？-->
<!--      </div>-->
<!--      <div slot="footer" class="dialog-footer">-->
<!--        <el-button @click="userActivateDialogFormVisible = false">取 消</el-button>-->
<!--        <el-button type="primary" @click="userActivate">确 定</el-button>-->
<!--      </div>-->
<!--    </el-dialog>-->
<!--    <el-dialog title="禁用用户" :visible.sync="userInactivateDialogFormVisible" width="35%" :close-on-click-modal="false">-->
<!--      <div>-->
<!--        确定禁用用户吗？-->
<!--      </div>-->
<!--      <div slot="footer" class="dialog-footer">-->
<!--        <el-button @click="userInactivateDialogFormVisible = false">取 消</el-button>-->
<!--        <el-button type="primary" @click="userInactivate">确 定</el-button>-->
<!--      </div>-->
<!--    </el-dialog>-->
...
export default {
  ...
  data() {
    return {
      ...
      // userActivateDialogFormVisible: false,
      // userInactivateDialogFormVisible: false,
  ...
  methods: {
    ...
    async getList() {
      ...
      console.log("888 getAllAdminsByDevApi ===this.parms===", this.parms);
      let res = await getAllAdminsByDevApi(this.parms);
      console.log(res);
    ...
    addBtn() {
      ...
      //设置是新增还是编辑
      this.addModel.roleFlag = "ROLE_ADMIN"
      this.addModel.editType = "0";
      this.readOnly = false;
      // this.isPswRequired = true;
    ...
    async userInactivateBtn(row) {
      console.log("0808 userInactivateBtn row", row);

      //表单的清空
      this.$resetForm('form', this.form)
      // this.userInactivateDialogFormVisible = true;
      this.form.status = '0'
      // row.status = '0'
      this.form.username = row.username

      //信息确认提示
      let confirm = await this.$myconfirm('确定禁用此用户吗?')
      console.log(confirm)
      if(confirm){
        console.log("===this.user===", this.user);
        console.log("===this.form===", this.form);
        let res = await updateUserStatusApi(this.form);
        if (res && res.code == 200) {
          row.status = '0'
          this.$message.success("用户禁用成功")
        } else {
          console.log("888 updateUserStatus===res===", res);
          this.$message.error(res.msg)
        }
        // this.userInactivateDialogFormVisible = false;
      }
      this.load
    ...
    // userInactivateBtn(row) {
    //   console.log("0808 row", row);
    //   //表单的清空
    //   this.$resetForm('form', this.form)
    //   this.userInactivateDialogFormVisible = true;
    //   this.form.status = '0'
    ...
    async userActivateBtn(row) {
      console.log("0808 userActivateBtn row", row);
      //表单的清空
      this.$resetForm('form', this.form)
      // this.userActivateDialogFormVisible = true;
      this.form.status = '1'
      // row.status = '1'
      this.form.username = row.username

      //信息确认提示
      let confirm = await this.$myconfirm('确定激活此用户吗?')
      console.log(confirm)
      if(confirm){
        console.log("===this.user===", this.user);
        console.log("===this.form===", this.form);
        ...
        // this.userActivateDialogFormVisible = false;
      }
      this.load
    },
